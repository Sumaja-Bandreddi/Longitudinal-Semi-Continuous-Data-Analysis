...
simulate_tobit_probit <- function(seed) {
  ...
  # If latent outcome falls below c, it is censored at c
  Y_ij    <- ifelse(Y_star <= c, c, Y_star)
  ...
  dat$Ipos <- as.integer(dat$Y_ij_final > 0)
  
  # --------------------------
  # 8) Fit two-part model in brms
  # --------------------------
  fit <- brm(
    bf(Ipos ~ t + (1 | p | id), family = bernoulli(link = "probit")) +
      bf(
        Y_ij_final | subset(Ipos == 1) + cens(cens_final) ~ t + (1 | p | id),
        family = lognormal()
      ),
    data   = dat,
    chains = 4,
  ...
  res <- as.data.frame(
    ...
    "b_Yijfinal_Intercept",
    "b_Yijfinal_t",
    "sd_id__Yijfinal_Intercept",
    "cor_id__Ipos_Intercept__Yijfinal_Intercept",
    "sigma_Yijfinal"),
...
# Save results
# -------------------------------
dir.create("results_zero_hurd", showWarnings = FALSE)

write.csv(
  res,
  file = paste0("results_zero_hurd/sim_", task_id, ".csv"),
  row.names = FALSE
)
...
summary_table <- all_sim %>%
  ...
  mutate(
    ...
    relative_bias = ifelse(true_value == 0, NA, bias / true_value)
  ) %>%
  select(
    parameter,
    true_value,
